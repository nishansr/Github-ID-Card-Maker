...
const ResultScreen = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setSelectedCard(<Card1 userData={data} userStats={{ totalStars, topLanguages, totalRepos: repos.length }} qrCode={qrUrl} options={cardOptions} />);
      } catch (error: any) {
  ...
  const generateAPILink = () => {
    ...
    const type = cardTypeMap[selectedCardType as keyof typeof cardTypeMap] || "1";
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <p className="text-xl font-semibold gradient-text">Loading your awesome card...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <div className="text-center glass-effect p-8 rounded-2xl">
          <h1 className="text-4xl font-bold gradient-text mb-4">User not Found!</h1>
          <p className="text-gray-600 dark:text-gray-400 mb-6">The GitHub user you're looking for doesn't exist.</p>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3">
            <div className="glass-effect p-6 rounded-2xl sticky top-8">
              <h3 className="text-xl font-bold gradient-text mb-6">Customize Your Card</h3>
              
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Show QR Code</label>
                  <button
                    onClick={() => toggleOption('showQR')}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      cardOptions.showQR ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                  >
                    <span
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Show Stats</label>
                  <button
                    onClick={() => toggleOption('showStats')}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      cardOptions.showStats ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                        cardOptions.showStats ? 'translate-x-6' : 'translate-x-1'
                      }`}
                    />
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Show Bio</label>
                  <button
                    onClick={() => toggleOption('showBio')}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      cardOptions.showBio ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                ...
                <div className="flex items-center justify-between">
                  <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Show Followers</label>
                  <button
                    onClick={() => toggleOption('showFollowers')}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      cardOptions.showFollowers ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                        cardOptions.showFollowers ? 'translate-x-6' : 'translate-x-1'
                      }`}
                    />
                  </button>
                </div>
              ...
              <div className="mt-8">
                <h4 className="text-lg font-semibold gradient-text mb-4">Share Your Card</h4>
          ...
          <div className="lg:col-span-9">
            ...
            <div className="mb-8">
              <h3 className="text-2xl font-bold text-center mb-6 gradient-text">Choose Your Style</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { name: "Card1", component: Card1, title: "Dark Professional", desc: "Sleek dark theme with purple accents" },
                  { name: "Card2", component: Card2, title: "Clean Modern", desc: "Minimalist design with colorful header" },
                  { name: "Card3", component: Card3, title: "Nature Inspired", desc: "Elegant emerald theme" },
                ].map((template) => (
                  <div
                    key={template.name}
                  ...
                  >
                    <div className="glass-effect p-4 rounded-2xl">
                      ...
                      <div className="text-center">
                        <h4 className="font-bold text-lg gradient-text">{template.title}</h4>
